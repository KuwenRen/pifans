...
//获取最近$day区间间的日期
function get_day_between($day = 7, $time = '', $format = 'Y-m-d')
{
    $time = $time != '' ? $time : strtotime('-1 day', time());
    //组合数据
    $date = [];
    for ($i = 1; $i <= $day; $i++) {
...
//用户邀请记录
function user_invite_record($user_id, $to_user_id)
{
    $powder_value = config('site.powder_value');
    $before_value = \app\common\model\User::where('id', $user_id)->value('powder_value');
    $data = [
        'user_id' => $user_id,
        'to_user_id' => $to_user_id,
        'powder_value' => $powder_value,
        'before_value' => $before_value,
        'after_value' => $before_value + $powder_value,
        'total_value' => $before_value + $powder_value
    ];
    \app\admin\model\InviteRecord::create($data);
    User::where('id', $user_id)->setInc('powder_value', $powder_value);
    return;
}

//获取五种头像
function get_five_avatar()
{
...
function host()
{
    ...
    $server_name = $url . $_SERVER['HTTP_HOST'] . ':' . $_SERVER["SERVER_PORT"];
    return $server_name;
}

//校验code
function check_code($account, $code, $event)
{
    $where['account'] = $account;
    $where['event'] = $event;
    $where['status'] = 0;
    $last_res = \app\common\model\Sms::where($where)->order('id desc')->find();
    if (!$last_res) {
        return 0;
    } else {
        if ($last_res['code'] != $code) {
            return 0;
        } else {
            //更新该记录
            \app\common\model\Sms::where('id', $last_res->id)->update(['status' => 1]);
            return 1;
        }
    }
}
...
//写入sms表
function create_sms($account, $event, $code)
{
    \app\common\model\Sms::create([
        'event' => $event,
        'account' => $account,
        'code' => $code,
        'ip' => request()->ip()
...
//用户编号
function user_number_bak()
{
    $prefix = 'P';
    $prefix .= \fast\Random::numeric(8);
    return $prefix;
}

//用户编号
function user_number($num)
{
    $bit = 8;//产生7位数的数字编号
    $num_len = strlen($num);
    $zero = '';
...
{
    $url = "http://restapi.amap.com/v3/ip?key=439ff69501eb204e1563cbf3eeddb07a&ip=" . $ip;
...
function getCity($ip = '')
{
    $appkey = "e1c60d80cdf7df5d36954c0d1f90c456";
...
{
    $key = "439ff69501eb204e1563cbf3eeddb07a";
...
{
    $key = '439ff69501eb204e1563cbf3eeddb07a';
    $url = "http://restapi.amap.com/v3/geocode/regeo?output=json&location=" . $address . "&key=" . $key;
...
//城市直播间绑定
function country_room_id($country_name, $user_id)
{
    //判断是否城市是否存在
    $huan = new \fast\Huan();
    if (!$chat_room = \think\Db::name('country_chat_room')->where('name', $country_name)->find()) {
        //直播间不存在,直接注册
        $res = $huan->createChatRoom([
            'name' => $country_name,
            'description' => $country_name . '直播间',
            'owner' => "2"
        ]);
        $res = json_decode($res, true);
        $chat_room_id = $res['data']['id'];
        //写入
        \think\Db::name('country_chat_room')->insert([
            'name' => $country_name,
    ...
    //将当前用户注册到该直播间里
    $huan->adduserChatRoom($user_id, $chat_room_id);

    //更新用户头像
    $avatar = \think\Db::name('user')->where('id', $user_id)->value('avatar');
    $huan->hx_avatar($user_id, ['avatarurl' => host() . $avatar]);
