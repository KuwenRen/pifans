<template>
	<view class="article container">
		<u-navbar :title="$t('hot')" bgColor="#14142B" leftIconColor="#F585FF" autoBack fixed placeholder />
	...
	export default {
		...
		methods: {
			getListHandler() {
				...
				(this.status !== 'nomore') && getHotInformationList({
					page: page + 1,
					limit: 12
				}).then(res => {
					if (res.code == 1) {
						if (res.data.length == 0) {
							...
							self.page = page + 1;
							self.list = list.concat(res.data);
						}
					} else {
						uni.$u.toast(res.msg);
					}
