<template>
	<u-list class="give container" @scrolltolower="getListHandler">
		<u-navbar :title="$t('my.like')" bgColor="#14142B" leftIconColor="#F585FF" autoBack fixed placeholder />
		<view class="warp">
			<sy-tabs type="custom" size="large" :data="config" @tabChange="tabChange"></sy-tabs>
			<view class="give-header">
				<view class="info">
					<view class="lable">{{$t('my.totalLikes')}}</view>
					<view class="count">{{count}}</view>
				</view>
				<u-image src="/static/img/my/give-banner.png" width="104rpx" height="104rpx" mode=""></u-image>
			</view>
			<view class="give-list" v-if="list.length !== 0">
				<view class="give-item" v-for="(item,index) in list" :key="index">
	...
	export default {
		data() {
			return {
				active: 'Information',
				config: [
					{ key:'Information', name: 'my.newsLikes' },
					{ key:'Knowledge', name: 'my.skillsLikes' }
				],
				status: 'none',
				count: 0,
				list: [],
		...
		methods: {
			getListHandler() {
				let { list, page } = this;
				(this.status !== 'nomore') && getRecordApi(this.active, { type: 2, page: page + 1, listRows: 10 }).then(({code, data:{ data, total }, msg}) => {
					if (code == 1) {
						if (data.length == 0) {
							this.status = 'nomore';
						} else {
