<template>
	<view class="page2">
		<u-navbar :title="name=='English'?'English Chat':$t(name+'聊天室')" bgColor="#14142B" leftIconColor="#F585FF" autoBack />
		<!-- <view class="loading-wapper" v-if="isLoading">
		...
		<view class="mycontent">
			...
			<scroll-view @scrolltoupper="scrolltoupper" :class="['content-wapper',showTost?'hideContent':'showContent']" :scroll-into-view="scrollId" scroll-y="true" @click="tabContent" :scroll-top="scrollTop">
				<view class="msg-wapper">
					<msg @tabImg="tabImg(item.content)" :id="'scrollId'+item.id" v-for="(item,index) in msgList" :key="index" :data="item"></msg>
				</view>
			</scroll-view>
		</view>
		<view class="bottom">
			<view class="input-wapper">
				<view class="input-box">
					<textarea placeholder-style="color:#FFF" v-model="text" :adjust-position="true" class="input" :auto-height="true" type="text" value="" :placeholder="$t('输入新消息')"/>
				</view>
				<text :class="['iconfont icon-icon_sent',text.trim()?'icon-icon_sent-ac':'']" @touchend.prevent="tabSend">
				</text>
			</view>
...
<script>
	...
	const expression = [ "😖","😠","😩","😲","😞","😵","😰","😒","😍","😤","😜","😝","😋","😘","😚","😷","😳","😃","😆","😁","😂","😄","😢","😭","😨","😣","😡","😌","😔","😱","😪","😏","😓","😥","😫","😉","😺","😸","😹","😽","😻","😿","😼","🙀","🐎","🐔","🐗","🐫","🐘","🐨","🐒","🐑","🐙","🐚","🌴","🌵"];
	import {gatewayserver_bind,gatewayserver_send,getChatRecordList} from '../../api/index.js'
	export default {
		data() {
			return {
				name:"",
		...
		onLoad({id,type,name}) {
			...
			this.getUser()
			uni.connectSocket({
				url: "wss://www.pifan.club:7272"
			});
			uni.onSocketMessage(data=>{
				console.log(JSON.parse(data.data))
				this.onMsg(JSON.parse(data.data))
			})
		},
		async onReady() {
		...
		methods: {
			...
			async getRecord(init){
				let res = await getChatRecordList({
					listRows:40,
					room_id:this.id,
