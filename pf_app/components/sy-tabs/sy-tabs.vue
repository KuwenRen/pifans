<template>
	<view class="sy-tabs" >
		<div class="sy-tabs__header" :class="`sy-tabs__header-${size}`">
			<view class="header-item" :class="{active: index == active}" v-for="(item, index) in data" :key="index" @click="tabIndex(index)">
				{{$t(item.name)}}
			</view>
		</div>
		<div v-if="type == 'default'" class="sy-tabs__body" :class="{none: !data[active].list || data[active].list.length == 0, grid: grid }">
